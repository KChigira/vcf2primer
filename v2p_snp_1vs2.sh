...
Usage: $(basename "$0") -R referance_genome.fasta -I input.vcf [OPTION]...
  ...
  -t [primer3 template file]    default:"template_snp.txt"
EOM
  exit 2
}
...
SCOPE=140
MARGIN=10
TEMP="template_snp.txt"

while getopts ":R:I:n:d:p:b:s:m:t:h" optKey; do
  case "$optKey" in
    R) REF=${OPTARG};;
    I) VCF=${OPTARG};;
    n) NAME=${OPTARG};;
...
perl perl/select_snp_1vs2.pl ${VCF} ${CURRENT}/${NAME}_selected.vcf \
...
while read line
do
  samtools faidx -n 10000 ${REF} ${line} \
